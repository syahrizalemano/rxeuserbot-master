...
from RXE import ALIVE_NAME, CMD_HELP, CMD_LIST
from RXE.modules import inlinestats

PMPERMIT_PIC = os.environ.get("PMPERMIT_PIC", None)
if PMPERMIT_PIC is None:
    WARN_PIC = "https://telegra.ph/file/beda314830f1a83d70c79.jpg"
else:
    WARN_PIC = PMPERMIT_PIC
LOG_CHAT = Config.PRIVATE_GROUP_ID
DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "RXE"
...
async def inline_handler(event):
    ...
    result = None
    query = event.text
    if event.query.user_id == bot.uid and query.startswith("RXE"):
        rev_text = query[::-1]
        ...
        result = builder.article(
            title="Stats",
            text=f"**Showing Stats For {DEFAULTUSER}'s RXE** \nNote --> Only Owner Can Check This \n(C) [RXE](t.me/ezzraez)",
            buttons=[
                [custom.Button.inline("Show Stats ?", data="terminator")],
                [Button.url("MANAGE BY", "t.me/ejjraa")],
                [Button.url("SUPPORT", "t.me/rahkissyou")],
            ],
        )
        await event.answer([result])
        ...
        result = builder.photo(
            ...
            text=query,
            buttons=[
                [custom.Button.inline("Spamming", data="dont spam")],
                [
...
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == bot.uid:
        ...
        reply_popp_up_alert = "Dibatalkan, saya tidak mengenalmu!"
...
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == bot.uid:  # pylint:disable=E0602
        ...
        reply_pop_up_alert = "Maaf, saya tidak mengenalmu.!"
...
async def on_plug_in_callback_query_handler(event):
    if not event.query.user_id == bot.uid:
        sedok = "Maaf saya tidak mengenalmu.."
    ...
    reply_pop_up_alert += "\n\n**(C) [RXE](t.me/ezzraez)** ".format(plugin_name)
...
async def rip(event):
    ...
    text1 = "Anda tidak datang untuk tujuan yang baik.. Pilihan Anda tidak dapat diterima.. Oleh karena itu Anda akan diblokir"
    await event.edit("Pilihan Anda tidak dapat diterima❌")
    await borg.send_message(event.query.user_id, text1)
    await borg(functions.contacts.BlockRequest(event.query.user_id))
    await tgbot.send_message(
        LOG_CHAT,
        f"HALLO master saya telah Menghukum[BLOCK](tg://user?id={him_id}) diblokir karena dia memilih larangan",
    )
...
async def sed(event):
    ...
    buttons = paginate_help(0, CMD_HELP, "helpme")
    sed = f"""RXE Modules Are Listed Here !\n
...
async def rip(event):
    if event.query.user_id == bot.uid:
        sedok = "Master, Anda tidak perlu menggunakan ini."
        await event.answer(sedok, cache_time=0, alert=True)
        return
    await event.get_chat()
    him_id = event.query.user_id
    await event.edit("Saya menerima pilihan Anda ✔️")
    text2 = "Oke sekarang tunggu sampai pemilik saya meninggalkan pesan untuk Anda.  \ nTerima kasih banyak telah mengirim PESAN.."
...
async def rip(event):
    if event.query.user_id == bot.uid:
        sedok = "Master, Anda tidak perlu melakukan ini."
        await event.answer(sedok, cache_time=0, alert=True)
        return
    await event.get_chat()
    him_id = event.query.user_id
    await event.edit("Saya menerima pilihan Anda ✔️")
    text3 = "Oke sekarang tunggu sampai pemilik saya meninggalkan pesan untuk Anda.  \ nTerima kasih banyak telah mengirim PESAN."
