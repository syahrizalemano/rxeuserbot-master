...
async def ban(event):
    ...
    if kekme.is_admin:
        await event.reply("Oh, Yeah? Lets Start Banning Boss.")
        return
    if user:
        pass
    else:
    ...
    try:
        ...
        await event.reply("Perintah di batalkan, anda tidak memiliki izin ban.")
        return
    if reason:
...
async def nothanos(event):
    ...
    try:
        ...
        await event.reply("`Unbanned Success, kesempatan diberikan.`")
    except BadRequestError:
        await event.reply("Perintah dibatalkan, anda tidak memiliki izin unban.")
        return
...
async def promote(event):
    ...
    kekme = await serena.get_permissions(event.chat_id, user)
    if kekme.is_admin:
        await event.reply("Mode promote start")
        return
    if not rank:
        rank = "Admin"
    if user:
    ...
    try:
        ...
        await event.reply("Promoted Successfully! Welcome admin")
    except BadRequestError:
        await event.reply("Perintah dibatalkan, anda tidak memiliki izin promote admin")
        return
...
async def demote(event):
    ...
    try:
        ...
        await event.reply("perintah di batalkan, anda tidak memiliki hak demote")
        return
...
async def pin(event):
    ...
    is_silent = True
    if options.lower() == "loading":
        is_silent = False
    try:
        await event.client(UpdatePinnedMessageRequest(event.to_id, to_pin, is_silent))
    except BadRequestError:
        await event.reply("Perintah dibatalkan, anda tidak memiliki hak pined.")
        return
...
async def kick(event):
    ...
    momos.first_name
    if kekme.is_admin:
        await event.reply("Mode kick actived.")
        retur
    if not user:
        await event.reply("Balas k pengguna")
        return
    try:
        await event.client.kick_participant(event.chat_id, user.id)
    except:
        await event.reply("Perintah dibatalkan, anda tidak memiliki hak kick.")
        return
    if reason:
        await event.reply(
...
async def mute(event):
    ...
    momos.first_name
    if kekme.is_admin:
        await event.reply("Muting mode on.")
        retur
    if not user:
        await event.reply("Balas ke pengguna")
        return
    try:
        await event.client(EditBannedRequest(event.chat_id, user.id, MUTE_RIGHTS))
    except:
        await event.reply("Perintah di batalkan, anda tidak memiliki hak mute.")
        return
    if reason:
        await event.reply(
...
async def mute(event):
    ...
    try:
        ...
        await event.reply("Perintah di batalkan, anda tidak memiliki hak unmute.")
        return
    if reason:
        await event.reply(
